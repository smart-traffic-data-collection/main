...
if __name__ == "__main__":
    print("Initializing UrbanIng 3D-to-2D Batch Projection...\n")
    
    # Iterate through every folder in the dataset directory
    for seq_folder in sorted(os.listdir(DATASET_DIR)):
        seq_path = os.path.join(DATASET_DIR, seq_folder)
        
        # Only process directories that look like sequence folders
        if not os.path.isdir(seq_path) or not seq_folder.startswith("20241126_"):
            continue
            
        print(f"➔ Scanning Sequence: {seq_folder}")
        ...
        # Extract sequence identifier (e.g., "0004" from "20241126_0004_crossing2_00")
        seq_num = seq_folder.split('_')[1] 
        
        # Find all thermal camera subfolders in this sequence
        for cam_folder in os.listdir(seq_path):
            if 'thermal_camera' in cam_folder:
                cam_path = os.path.join(seq_path, cam_folder)
                
                # Construct exact output path to match your structure
                out_dir = os.path.join(OUTPUT_BASE, f"labels_{seq_num}", f"yolo_{cam_folder}")
    ...
    print("\nBatch projection completely finished. YOLO datasets are ready.")